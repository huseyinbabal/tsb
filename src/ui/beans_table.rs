...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    ...
    let filtered: Vec<(usize, &crate::model::Bean)> = app
        .beans
        .iter()
        .enumerate()
        .filter(|(_, b)| {
    ...
    let title = if filter.is_empty() {
        format!(" Beans ({}) ", app.beans.len())
    } else {
        format!(" Beans ({}/{}) ", filtered.len(), app.beans.len())
    };
    ...
    if filtered.is_empty() {
        let msg = if app.beans.is_empty() {
            "  No beans loaded. Connect to a server and run :beans"
        } else {
            "  No matching beans."
        };
    ...
    let rows: Vec<Row> = filtered
        ...
        .map(|(_, b)| {
            ...
            Row::new(vec![
                Cell::from(highlight_text(&b.name, &filter, base)),
                Cell::from(highlight_text(&b.scope, &filter, base)),
                Cell::from(highlight_text(&b.type_name, &filter, base)),
            ])
        })
        .collect();
    ...
    let selected_pos = filtered
        .iter()
        .position(|(idx, _)| *idx == app.selected_bean_index)
        .unwrap_or(0);
